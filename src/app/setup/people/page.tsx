      ...
      <ul className="max-w-3xl divide-y rounded-lg border bg-white">
        {roles.map(r => (
          <li key={r.id} className="flex flex-wrap items-center gap-3 p-3 text-sm">
            <div className="w-56"><b>{r.title}</b><div className="text-xs text-ink-light">{r.level} · {r.level === 'staff' ? 'read-only' : 'full access'}</div></div>
            {r.holder
              ? <div className="flex-1 text-slate-700">{r.holder.name} <span className="text-ink-light/60">· {r.holder.email}</span></div>
              : <form action={assign} className="flex flex-1 gap-2"><input type="hidden" name="roleId" value={r.id} />
                  ...
                  <button className="rounded bg-rust px-3 py-1 text-white hover:bg-rust-dark">Assign and invite</button></form>}
            {r.holder && <form action={assign} className="flex gap-2"><input type="hidden" name="roleId" value={r.id} />
              ...
              <button className="text-xs text-ink-light hover:text-rust">Reassign</button></form>}
          </li>
        ))}
      </ul>
      <p className="mt-4 max-w-3xl text-xs text-ink-light">Local demo: invites are recorded, not emailed. The invited person signs in with their email and lands on their own scorecard. Reassigning moves the person; it never edits the role.</p>
