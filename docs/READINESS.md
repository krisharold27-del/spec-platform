...
| What | Evidence |
|---|---|
| The engine's arithmetic | 1284 tests across 82 files. Pillar, role and team maths, the 90% rule, incentive ceilings, the deduction and its cap, the seven statuses, the rule of 8 — all measured against `designs/the-rules.md` |
...
| Every table is isolated in the database too | `npm run db:check-rls` applies the real policy file and then asks Postgres what it actually got. **34 of 34** |
...
23, and it is now **34 of 34**, with `rulebook_rules` global by design and
