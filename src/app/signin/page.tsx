...
export default async function SignIn({ searchParams }: { searchParams: Promise<{ unknown?: string; exists?: string; sent?: string; error?: string }> }) {
  ...
  const sendError = sp.error ? SEND_ERRORS[sp.error] ?? SEND_ERRORS.failed : null;
  return (
    <main className="mx-auto max-w-md px-6 py-16">
      ...
      {sendError && <p className="mt-3 rounded border border-red-200 bg-red-50 p-3 text-sm text-red-900">{sendError}</p>}
      {sp.sent ? (
