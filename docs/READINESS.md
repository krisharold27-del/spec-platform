...
| What | Evidence |
|---|---|
| The engine's arithmetic | 2448 tests across 139 files. Pillar, role and team maths, the 90% rule, incentive ceilings, the deduction and its cap, the seven statuses, the rule of 8 — all measured against `designs/the-rules.md` |
...
| **A tradie can do their own half on their phone, with no account** | `scripts/join-journey.mjs` (26 checks, driven at 390×844 with touch — an iPhone in one hand, which is the only size that proves anything about this page) and `tests/join-page.test.ts`. Kris, 24 September: the office does the structure and the money, the person's half gets done *"on their phone"*. The reason is not convenience: **the certificate is in their wallet**, and an admin typing thirty-eight people's licences off photocopies gets the expiry dates wrong — which is the one field that has to be right, because it is what stops somebody being booked the day after their ticket lapses. The office screen had been issuing these links and showing their progress since the 24th, and the link went nowhere, which made the whole feature a promise. The journey sends the link from the office at desk size, then opens **the link the admin was actually handed** — read off the screen, not assembled by the script, because building the URL here would prove the token exists and nothing about whether somebody working down thirty-eight rows can send one. It found three things. `'use server'` files may only export async functions, which typecheck does not know and the browser did. The person could see their business's name upper-cased by CSS — wrong for the one line doing security work, since what makes a text from an unknown number not look like a scam is seeing your employer's name spelled the way they spell it. And the fence check ran against a business with nobody else in it, so it passed having proved nothing; it now fails loudly if there is no second person to try to reach. The fence itself holds: **a person may fill in their own details and may not mark themselves inducted**, because a link that arrives as a text can be forwarded, and inducting yourself is how an uninducted body walks onto a site |
| **Three quiet leaks, closed: agreed rates, plant on hire, and somebody’s last day** | `scripts/leaks-journey.mjs` (21 checks in a real browser) and `tests/last-three.test.ts`. The workflow map’s last three holes, taken together because they are the same shape — money that leaves because nothing said it was going. A builder’s schedule typed from memory drifts, and only ever one way, and nobody finds out: a quote priced off the wrong rate is not rejected, it is accepted, and the difference comes out of the job. Plant runs past the job that needed it because off-hiring is nobody’s specific job — so **the job finishing is the trigger**, not a reminder anybody sets, and the waste is counted from the day the JOB ended rather than the day it went out, because conflating them makes the number enormous and therefore ignorable. Offboarding already existed in six places with nothing joining them, so it was done by whoever remembered, which on a last day is nobody; two of the six bite and the summary leads with those — a login that still works, and a seat still being paid for. **SPEC closes nothing itself**: somebody on gardening leave still has a login on purpose. The journey found a real pre-existing bug — `refuseTo` appended `?cannot=` even when the screen already had a query, so every refusal on `/people?mode=setup` produced an address with two question marks and silently dropped the person onto a different tab. Nothing failed, which is why it survived: landing somewhere odd after a refusal reads as SPEC being confusing rather than as a bug |
| **The document that proves the work was lawful lives on the job** | `scripts/certificate-journey.mjs` (17 checks in a real browser) and `tests/certificates.test.ts`. From the workflow review: a job finishes, the customer signs it off on the phone, the invoice goes — and the certificate of compliance is done somewhere else entirely, on a different system or a pad in the ute. The first time anybody goes looking is an insurance claim, a fire, a regulator or a builder’s audit. **SPEC does not know what yours is called or when it is due, and says so**: every state runs its own scheme under its own name inside its own window and they change, so there is not a single day count in `lib/certificates` and a test fails the build if one appears. A business that has not set a window is told the certificate is outstanding and explicitly NOT told whether it is late — a deadline SPEC invented would be worse than no deadline, because a business lodging to a made-up window has been misled by the thing it trusted to keep it right. Written and LODGED are two presses, because the gap between them is where businesses fall: the customer has their copy, everybody believes it is done, and the body that had to receive it never did. The journey found a real one — the summary counted “written but not sent” off the state label, so the group in the worst trouble (written, in a drawer, clock run out) was the one group it never mentioned — and one of its own checks printed the navigation instead of the row it was testing, which is a failure message nobody reads twice |
| **A crew locked out is counted, and the customers who do it twice are named** | `scripts/no-access-journey.mjs` and `tests/no-access.test.ts`. The most common thing that wrecks a day, and the thing almost no trade business counts — because counting it meant ringing the office and somebody typing a note later, so it disappeared into the day instead. One press on the phone records it, tells the customer there is no charge, and puts the job back on the list to rebook. **The reason is offered afterwards and never required**: the reason is the part that stops it being pressed, and a record only made on quiet days produces a number saying quiet days have the most no-access. The figure that changes a decision is not the total — it is which customers do it repeatedly, ranked by how often rather than how recently, because a list sorted by date puts whoever did it most recently on top and that is almost never the person worth ringing |
| **A job split between crews has one person carrying it** | `scripts/crews-journey.mjs` (14 checks in a real browser) and `tests/crews.test.ts`. Kris, 25 September: *"multi crew is common - split scopes with one supervisor overall"*. The booking half already worked and nobody had noticed — the schedule's uniqueness rule is on person-and-day rather than on job, so any number of people could already be put on one job and nobody double-booked. What was missing was the second half: anywhere to record who carries the WHOLE thing. A job split into scopes runs fine right up until the scopes disagree — the switchboard crew waiting on the lighting crew, somebody having to decide which one moves, the customer asking one person about all of it — and that is exactly the day a system with a lead-per-scope has nobody to point at, because every lead is answerable for their own part and none of them for the join. **The second scope is the declaration**: nobody has to tick “this is a multi-crew job”, and a single crew on a single job is never asked to nominate itself, which is the kind of ceremony that teaches people the software is not on their side |
| **Going back paid is a job; going back free is rework** | `scripts/rework-journey.mjs` (10 checks in a real browser) and `tests/rework.test.ts`. Kris, 25 September: *"if returning for troubleshooting etc and paid this is just a continuation of the job - anything paid simple job process - unpaid is re work and negative to the business"*, and before it: *"anything re work or call back that isnt paid is a key power meter detractor"*. Every return visit used to become a callback and SPEC then asked what caused it — which put the hard question first, on a screen somebody fills in at the end of a long day, and counted PAID work as rework, so a business that goes back often and charges for it read as a business with a quality problem. The first question is now one word nobody has to think about: is this being paid for? Paid writes billable hours on the job and never reaches the rework screen; unpaid is what rework IS, and only then is the cause asked. **The journey found the thing no unit test could**: `logCallback` wrote a flat `costCents: 0` on every row, so the unpaid-rework figure was structurally incapable of ever being anything but zero — the model was right, its tests were right, and nothing on earth was going to put a value in. It is now costed from the business's own cheapest labour rate, and a business that has not set one is told hours rather than being shown an invented dollar figure |
| **A subcontractor never sees your margins, and nor does a team member** | `scripts/sight-journey.mjs` (32 checks in a real browser) and `tests/sight.test.ts`. Kris, 25 September: *"they can use the app the same as an employee - everything is simple and direct through the app for subbies"* — which is about HOW the work is done, and prompted the question of what comes with it. Two answers: a subcontractor sees **their own work only**, and for employees, **money is leadership**. Until then `/jobs` was gated on one thing — are you signed in to this business. `canManage` decided whether the BUTTONS appeared and every tab rendered for everybody, so any login could open Work in progress and read every job’s margin, Cash flow and see the whole position, and Customers and take the client list. **Nobody decided that**: it is what happens when permission is built as “may I press this” and sight is left to whatever the page happens to render — the buttons get guarded because they are visible in the code, and the numbers underneath them do not. A subbie is not disloyal; a subbie is somebody who very often also works for a competitor next week. The journey drives one account through three seats rather than three people, because creating a user by SQL gives them no account with the authentication provider and could never sign in — and it proves the thing that would actually leak: typing `?tab=cash` does not get past it, and a team member’s page **source** carries no job value, not merely a page that hides one |
| **Work keeps coming in without anybody remembering to look** | `scripts/growth-journey.mjs` (18 checks in a real browser, against real rows), `tests/growth.test.ts` and `tests/sectors.test.ts`. Kris, 24 September, shown that Growth was the least automatic of the three streams at 30% of its steps: *"growth automation gap - thats our weakest and most important"*. The reason it was weakest is worth keeping: **winning work feels like judgement, so it got built as screens to look at** — and a list somebody has to open is a list that does not get opened in the week everybody is flat out, which is the exact week that decides whether there is work in six. Debtors had chased themselves at 7, 14 and 30 days since September; quotes had not, and nobody decided that. Now a quote raises its own chase at 3, 7 and 14 days, each one written and each saying something different, and a quote nobody touched gets the chase that is DUE rather than the gentle first nudge eleven days late. Customers past their own rhythm surface on their own out of job history already held, ranked by what they have been worth rather than by how long they have been quiet — the longest-quiet list puts the smallest customers at the top, every time. It found two faults in its own checking: the chase label is upper-cased by CSS, so a case-sensitive check reported a failure against working code, and the follow-up check that the same chase is never offered twice then passed **having proved nothing**, because it could not match either. Growth is now 38%, earned rather than relabelled |
| **An HR admin can set a whole business up in one sitting** | `scripts/setup-journey.mjs` (18 checks in a real browser) and `tests/onboarding.test.ts`. Kris, 24 September, with an HR admin booked for Monday and thirty-eight JBI people to enter: *"finalise payment after every is set - have a tick or toggle to change from team member to leadership - and also want to see subcontractors"*, *"I will work through with hr admin on monday so make it simple and beautiful"*. The journey signs a business up, pastes a structure in, then works the screen the way the admin will: the Team/Leadership toggle, the Subcontractor tick, a company email, a licence with its expiry, marking somebody inducted, and sending the phone link — checking each one round-trips. It found two things. The screen had never been driven at all, and the first run reported ten failures that were the harness rather than the product: the check called a redirect to `/signin` an opened page, because a followed redirect answers 200. **Where you LAND is the fact that matters**, and it now asserts the pathname. The real fault was the one underneath: pressing **Send them a link** for somebody who had already finished their half confirmed nothing, because the line hung off there being a NEXT step. Working down thirty-eight people that reads as a broken button, and the link gets sent three more times |
...
| **Every core component is in the menu and opens** | `tests/core-components.test.ts` and `scripts/core-components-journey.mjs`, in a real browser. Kris, 25 September, could not find the org chart — it worked at `/org` and nothing led to it. `docs/CORE-COMPONENTS.md` lists what must always be findable (the org chart second in the menu, after My page); the unit test holds the menu's code to that table and the journey signs up a new business, presses every item, fails if one is missing or errors, and checks every item is still there on a phone. "All pages" now opens `/pages`, a real list, instead of the foot of My Page |
...
| Every table is isolated in the database too | `npm run db:check-rls` applies the real policy file and then asks Postgres what it actually got. **85 of 85**. On 24 September three tables went in — `sectors` (then called `work_streams`), `quote_chases`, `shutdowns` — and the deploy printed "69 tables carry the tenant policy", the same number as before, for a schema that had just grown by three: the policy file names its tables in hand-written lists, several test files checked THEIR tables, and nothing covered a new table nobody had written a test for. `tests/rls-coverage.test.ts` now derives the list from `schema.ts` and asks the question the other way round — for every table with a `tenant_id`, is there a policy? Proved by putting the fault back. Its first version was itself wrong in the way that matters most for a security check: it took a table's body as far as the next `pgTable(`, swallowing the prose between them, and reported `board_outputs` — which has no `tenant_id` at all — as unprotected. **A check that cries wolf is worse than no check**, because the first thing anybody does with a security test that names an innocent table is stop reading its output |
