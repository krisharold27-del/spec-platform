    ...
    <li className="mt-2">
      <div className="flex items-center gap-3 rounded-lg border bg-white px-4 py-3" style={{ marginLeft: depth * 28 }}>
        <div className="flex-1">
          <Link href={`/scorecard/${role.id}`} className="font-medium text-rust hover:underline">{role.title}</Link>
          <span className="ml-2 rounded bg-cream px-2 py-0.5 text-xs uppercase text-ink-light">{role.stream}</span>
        </div>
        <div className="text-sm text-ink-light">
          {role.holder ? <>{role.holder.name} <span className="text-xs text-ink-light/60">· {role.holder.access}</span></> : <span className="italic text-ink-light/60">vacant — role exists, nobody assigned</span>}
        </div>
      </div>
...
export default async function OrgChart() {
  ...
  return (
    <Shell title="Org chart" subtitle="Roles report to roles. A role can exist with nobody in it; a person cannot exist without a role.">
      ...
      <div className="mt-8 rounded-lg border bg-white p-4 text-sm text-ink-light">
        <div className="font-medium text-ink">Streams (COGS)</div>
