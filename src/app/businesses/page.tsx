...
export default async function Businesses() {
  ...
  return (
    <Shell title="Your businesses" subtitle={businesses.length === 1 ? 'You have one business on SPEC.' : 'Choose which one to open. SPEC remembers your choice on this browser.'}>
      <ul className="max-w-xl divide-y divide-ink/10 rounded-lg border border-ink/10 bg-surface">
        {businesses.map(b => (
          <li key={b.tenantId} className="flex items-center justify-between gap-4 p-4">
            ...
            {b.current
              ...
              : <form action={openBusiness}><input type="hidden" name="tenantId" value={b.tenantId} /><button className="rounded-full bg-rust px-4 py-2 text-sm text-cream hover:bg-rust-600">Open</button></form>}
          </li>
        ))}
      </ul>
    </Shell>
