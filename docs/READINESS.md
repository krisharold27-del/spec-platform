...
| What | Evidence |
|---|---|
| The engine's arithmetic | 1079 tests across 69 files. Pillar, role and team maths, the 90% rule, incentive ceilings, the deduction and its cap, the seven statuses, the rule of 8 — all measured against `designs/the-rules.md` |
...
| **A business that wants to pay can** | `scripts/pay-journey.mjs`, 38 checks in a real browser. On 16 September the answer was **no**: `/api/stripe/checkout` existed, worked, was unit-tested, and was reachable from exactly one button — "Fix payment", shown only after a subscription had already FAILED. A business with seats was shown its monthly cost and a Billing button that opened a portal needing a Stripe customer it had never had, and was silently redirected back to the same page. Every test passed; they all checked the rooms rather than whether you could get into the building. The journey now holds the invariant — **a business is either free or has a way to pay** — and was proven by putting the fault back: 5 of its checks fail on the old code |
...
| Every table is isolated in the database too | `npm run db:check-rls` applies the real policy file and then asks Postgres what it actually got. **33 of 33** |
...
| The product carries what the designs say | `npm run designs:coverage` — **174 of 174** headings and buttons and **472 of 472** labels, per design screen, named individually when one is missing. The percentage rounds DOWN and prints 100 only when every phrase is really there: it used to round to nearest and reported 470 of 472 as "100%", directly under the line naming the two it had not found. **One screen is deliberately outside that number** — `SPEC Boards`, which arrived in export 3 on 16 September and is a feature nobody has built, not a copy edit. It is named in `designs/not-built-yet.md` with the date and the reason, and the check shouts about it on every run, because the two wrong answers were letting a new screen turn the build red (which punishes the export) and filing it under `superseded.md` (which is for references, and would have hidden it) |
...
23, and it is now **33 of 33**, with `rulebook_rules` global by design and
