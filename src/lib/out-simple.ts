 ...
 * are deployed. The weekly one-page "Make it simple" report is live as the first item on /meeting
 * (lib/make-it-simple). Angus Shield keeps no books yet, so it is not switchable.
 */
export const FEATURES: Record<FeatureKey, Feature> = {
  ...
  make_it_simple: { key: 'make_it_simple', live: true, evidence: '/meeting' },
