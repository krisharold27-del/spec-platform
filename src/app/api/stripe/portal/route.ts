...
export async function POST() {
  ...
  /*
    ...
    This used to redirect to /billing (then /journey) with NOTHING SAID — the page reloaded, looked identical, and a
  ...
  */
  if (!tenant?.stripeCustomerId) return NextResponse.redirect(`${here}/billing?no_subscription=1`, 303);
  ...
  /*
    ...
    address on `/billing` for exactly this shape of failure.
  */
  try {
    const session = await stripe.billingPortal.sessions.create({
      customer: tenant.stripeCustomerId,
      return_url: `${here}/billing`,
    });
