...
export default async function Journey() {
  ...
  return (
    <Shell title={tenant.name} subtitle={`${shapeSentence(shape)} · ${costLabel(plan)}`}>
      {/*
        Money — what it costs, the AI-powered question, and Stripe billing — moved to `/billing`
        (nav label "Pricing") on 22 September; see the note there. `plan.readOnly` still needs a way
        forward from wherever somebody lands, so it stays as a pointer rather than disappearing.
      */}
      {plan.readOnly && (
        <div className="mb-4 flex items-center justify-between rounded-lg border border-rust-300 bg-rust-100 p-4 text-sm text-rust-800">
          ...
          <Link href="/billing" className="ml-4 shrink-0 rounded-full bg-rust-800 px-4 py-2 text-sm font-medium text-cream hover:bg-rust-900">
            Sort payment
          </Link>
        </div>
      )}
