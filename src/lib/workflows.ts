...
import { STREAM_KEYS, type Owner, type StreamKey } from './streams';
...
export interface Workflow {
  ...
  moves: SlotId[];
  /**
   * Which of the three streams is accountable for it, or `whole` for the seat above them.
   *
   * Kris, 24 September: *"there are three main streams in a successful trade business ... COGS -
   * Commercial ... Operations getting the work done safely and Growth making sure new work is
   * coming in steadily"*.
   *
   * ONE owner, never a list, and that is the point of the field. Work that belongs to everybody
   * belongs to nobody — the reason a quote sits unchased for three weeks is almost never that
   * nobody could chase it. `whole` is not an escape hatch for the awkward ones: it is the General
   * Manager's own work, holding the three together, and a test keeps it to that.
   */
  owner: Owner;
  steps: Step[];
}
...
const W = (
  ...
  owner: Owner, moves: SlotId[], steps: Step[],
): Workflow => ({ id, family, name, starts, ends, owner, moves, steps });
...
export const WORKFLOWS: Workflow[] = [
  ...
  W('win', 'phone-enquiry', 'The phone rings',
    ...
    'It is on the board with a name against it, or booked.', 'growth', ['revenue_growth', 'productivity'], [
  ...
  W('win', 'web-enquiry', 'An enquiry comes in online',
    ...
    'Same list as a phone call, with where it came from recorded.', 'growth', ['revenue_growth'], [
  ...
  W('win', 'quote-build', 'Build and send a quote',
    ...
    'Sent, with the margin known before it went.', 'growth', ['gross_profit', 'revenue_budget'], [
  ...
  W('win', 'quote-accept', 'The customer accepts',
    ...
    'A job exists, carrying the quote’s prices as its budget.', 'growth', ['revenue_budget', 'revenue_growth'], [
  ...
  W('win', 'quote-lost', 'Record why one was lost',
    'They went elsewhere.',
    'The reason is in the system, not in somebody’s head.', 'growth', ['revenue_growth'], [
  ...
  W('win', 'takeoff', 'Count off the plans',
    'A set of drawings.',
    'A priced quantity list you can quote from.', 'growth', ['gross_profit'], [
  ...
  W('win', 'maintenance-sale', 'Sell a maintenance agreement',
    ...
    'An agreement that raises its own jobs.', 'growth', ['revenue_growth', 'cash_flow'], [
  ...
  W('win', 'estimate-accuracy', 'Find out whether you price it right',
    ...
    'The next quote for that work is priced off what it really takes.', 'commercial', ['gross_profit', 'productivity'], [
  ...
  W('do', 'book-job', 'Put a job on the board',
    ...
    'It has a day, a crew and a customer who knows when.', 'operations', ['productivity', 'licensing'], [
  ...
  W('do', 'dispatch', 'Send the crew out',
    'The day starts.',
    'Everybody knows where they are going and the customer knows you are coming.', 'operations', ['productivity'], [
  ...
  W('do', 'prestart', 'Take 5 before anything starts',
    'Arriving on site.',
    'Five questions answered, and the job cannot start until they are.', 'operations', ['safety_incident', 'near_miss'], [
  ...
  W('do', 'swms', 'The SWMS for this job',
    ...
    'Signed by everybody on it before work starts.', 'operations', ['safety_incident', 'regulatory'], [
  ...
  W('do', 'site-induction', 'Get onto a builder’s site',
    ...
    'Everybody going has been inducted, and it is on record.', 'operations', ['training_done', 'licensing'], [
  ...
  W('do', 'day-on-phone', 'A tradie’s day',
    ...
    'Every job either finished with evidence, or handed on with a reason.', 'operations', ['productivity', 'gross_profit'], [
  ...
  W('do', 'photos', 'Photos from site',
    ...
    'On the job, visible to the business and nobody else.', 'operations', ['contract_breach', 'audit'], [
  ...
  W('do', 'variation', 'Extra work found on site',
    ...
    'Priced and agreed BEFORE it is done, with a name against it.', 'operations', ['gross_profit', 'contract_breach'], [
  ...
  W('do', 'sign-off', 'The customer signs it off',
    'The work is done.',
    'Signed, and the invoice can go.', 'operations', ['debtor_days', 'contract_breach'], [
  ...
  W('do', 'job-hold', 'A job stuck waiting on something',
    ...
    'It is visibly waiting, with a reason, instead of quietly rotting.', 'operations', ['cash_flow', 'productivity'], [
  ...
  W('do', 'subbie-in', 'You are the subbie',
    ...
    'You claim on their schedule and get paid.', 'operations', ['contract_breach', 'cash_flow'], [
  ...
  W('buy', 'supplier-pickup', 'Pick it up from the supplier',
    ...
    'It is on the job cost, not on a docket in a glovebox.', 'operations', ['gross_profit'], [
  ...
  W('buy', 'purchase-order', 'Raise a purchase order',
    ...
    'Ordered against the job, with the order on record.', 'commercial', ['gross_profit', 'budget_miss'], [
    { does: 'Raise the order against the job.', by: 'office', where: '/jobs?tab=stock' },
  ]),

  W('buy', 'supplier-bill', 'Match the supplier’s invoice',
    'The bill arrives.',
    'Paid, or held because it does not match the order.', 'commercial', ['budget_miss', 'gross_profit'], [
  ...
  W('buy', 'price-rise', 'The supplier puts prices up',
    'A new price file.',
    'The catalogue is repriced and every quote already out with the old price is named.', 'commercial', ['gross_profit'], [
  ...
  W('buy', 'van-stock', 'Keep the vans stocked',
    ...
    'A reorder list that becomes an order in one press.', 'operations', ['productivity'], [
  ...
  W('buy', 'stocktake', 'Count the stock',
    'Time to count.',
    'Counted, with the date on it.', 'operations', ['gross_profit'], [
  ...
  W('buy', 'catalogue-health', 'Keep the catalogue streamlined',
    ...
    'It stays fast, and the business is told before it does not.', 'operations', ['productivity'], [
  ...
  W('buy', 'tool-issue', 'Give somebody a tool',
    ...
    'It is known who has it and when it is next due for test.', 'operations', ['safety_actions', 'licensing'], [
  ...
  W('buy', 'test-and-tag', 'Test and tag',
    ...
    'Every item tested, with the result and the next date.', 'operations', ['regulatory', 'audit', 'safety_actions'], [
  ...
  W('buy', 'vehicles', 'Utes, trailers and plant',
    ...
    'Nothing on the road out of rego or unchecked.', 'operations', ['regulatory', 'safety_incident'], [
  ...
  W('people', 'onboard-employee', 'Set up a new employee',
    'Somebody starts.',
    'They can be booked on a job, and the seat is paid for.', 'operations', ['training_done', 'licensing', 'turnover'], [
  ...
  W('people', 'phone-setup', 'The person does their half on their phone',
    ...
    'Their licences and tickets are in, photographed from their own wallet.', 'operations', ['licensing', 'training_done'], [
  ...
  W('people', 'onboard-apprentice', 'Take on an apprentice',
    ...
    'Set up like anybody else, and the funding that applies is claimed.', 'operations', ['training_done', 'turnover'], [
  ...
  W('people', 'leave', 'Somebody wants time off',
    'A leave request.',
    'Approved or not, and the schedule knows.', 'operations', ['absenteeism'], [
  ...
  W('people', 'licence-expiry', 'A licence is about to run out',
    ...
    'Renewed before it stops somebody working.', 'operations', ['licensing', 'regulatory'], [
  ...
  W('people', 'training-assign', 'Somebody needs training',
    ...
    'Finished, and on record against the role.', 'operations', ['training_done', 'dev_plans'], [
  ...
  W('people', 'review', 'A performance review',
    ...
    'A conversation held on real numbers, recorded.', 'operations', ['engagement', 'dev_plans', 'turnover'], [
  ...
  W('people', 'warning', 'Something has to be addressed',
    ...
    'A fair process, every step recorded, none skipped.', 'operations', ['turnover', 'regulatory'], [
  ...
  W('people', 'hire', 'Fill an empty seat',
    ...
    'Somebody starts in it.', 'operations', ['turnover', 'dev_plans'], [
  ...
  W('safe', 'hazard', 'Report a hazard',
    ...
    'On the safety register with an owner and a date.', 'operations', ['near_miss', 'safety_actions', 'corrective'], [
  ...
  W('safe', 'near-miss', 'Report a near miss',
    'It nearly happened.',
    'Recorded and acted on, the same as if it had.', 'operations', ['near_miss', 'safety_actions'], [
  ...
  W('safe', 'injury', 'Somebody is hurt',
    ...
    'The business has been told, immediately, and the steps are underway.', 'operations', ['safety_incident', 'trifr', 'lti'], [
  ...
  W('safe', 'notifiable', 'A serious injury',
    ...
    'The regulator has been called and the site left as it is.', 'operations', ['regulatory', 'safety_incident'], [
  ...
  W('safe', 'workers-comp', 'A workers’ compensation claim',
    ...
    'Claim lodged on time and the person back on suitable duties.', 'operations', ['workers_comp', 'lti', 'absenteeism'], [
  ...
  W('safe', 'trifr', 'Know your TRIFR',
    ...
    'A real number from real hours, not an estimate.', 'operations', ['trifr', 'safety_incident'], [
  ...
  W('safe', 'corrective-action', 'Close out a corrective action',
    ...
    'Done, with a review date that came and was met.', 'operations', ['corrective', 'snap_score'], [
  ...
  W('safe', 'toolbox-talk', 'Run a toolbox talk',
    'A scheduled talk.',
    'Everybody who should have been there has signed on.', 'operations', ['safety_actions', 'training_done'], [
  ...
  W('safe', 'site-inspection', 'Inspect a site',
    ...
    'Done, with what it found turned into actions.', 'operations', ['audit', 'safety_actions'], [
  ...
  W('safe', 'psychosocial', 'Somebody is not coping',
    ...
    'Acted on, without ever knowing who made it.', 'operations', ['engagement', 'safety_incident'], [
  ...
  W('safe', 'clear-to-work', 'Nobody unclear goes out',
    ...
    'Either clear, or not bookable.', 'operations', ['licensing', 'regulatory'], [
  ...
  W('safe', 'insurances', 'Keep the business’s own insurances current',
    ...
    'Current, with the certificate on hand when a builder asks.', 'operations', ['regulatory', 'licensing'], [
  ...
  W('money', 'invoice-small', 'Invoice a small job',
    ...
    'Invoiced, and in the accounting system.', 'commercial', ['debtor_days', 'cash_flow'], [
  ...
  W('money', 'progress-claim', 'Claim on a big job',
    ...
    'Claimed and paid.', 'commercial', ['cash_flow', 'debtor_days'], [
  ...
  W('money', 'retention', 'Get the retention back',
    ...
    'Released when it is due, rather than forgotten.', 'commercial', ['cash_flow'], [
  ...
  W('money', 'debtor-chase', 'Chase the money',
    ...
    'Paid, with every chase recorded so the same one never goes twice.', 'commercial', ['debtor_days', 'cash_flow'], [
  ...
  W('money', 'job-costing', 'Did the job make money',
    ...
    'The real margin, while there is still time to do something.', 'commercial', ['gross_profit', 'budget_miss'], [
  ...
  W('money', 'cashflow', 'Will there be enough money',
    ...
    'A running balance, and a warning before it goes under the buffer.', 'commercial', ['cash_flow'], [
  ...
  W('money', 'reviews-ask', 'Ask for a review',
    'A job finished well.',
    'Asked — every customer, with no filtering.', 'growth', ['revenue_growth'], [
  ...
  W('run', 'weekly-meeting', 'The weekly meeting',
    ...
    'Decisions made and written down, not a discussion that repeats next week.', 'whole', ['snap_score', 'engagement'], [
  ...
  W('run', 'monthly-scorecard', 'Score the month',
    'The month ends.',
    'Every role scored, and the scores are what reviews and Aces read from.', 'whole', ['engagement', 'dev_plans', 'snap_score'], [
  ...
  W('run', 'org-chart', 'Who does what',
    ...
    'A chart where every role has a name and what it is measured on.', 'whole', ['dev_plans', 'turnover'], [
  ...
  W('run', 'board-pack', 'The board pack',
    'The board meets.',
    'One pack, built from what already happened.', 'whole', ['net_margin', 'revenue_budget'], [
  ...
  W('run', 'automation-review', 'What should be automated',
    ...
    'A queue of things worth building, in order.', 'whole', ['productivity', 'snap_score'], [
  ...
  W('run', 'switch-it-off', 'Turn the old system off',
    ...
    'Everything keeps working. Only new rows stop arriving.', 'whole', ['net_margin', 'productivity'], [
...
/** Everything one stream is accountable for. What a Commercial Manager's job actually IS. */
export const ownedBy = (owner: Owner, list: readonly Workflow[] = WORKFLOWS): Workflow[] =>
  list.filter(w => w.owner === owner);

export interface StreamTally { owner: Owner; total: number; whole: number; auto: number; steps: number }

export function byStream(list: readonly Workflow[] = WORKFLOWS): StreamTally[] {
  const owners: Owner[] = [...STREAM_KEYS, 'whole'];
  return owners.map(owner => {
    const ws = ownedBy(owner, list);
    return {
      owner,
      total: ws.length,
      whole: ws.filter(w => stateOf(w) === 'whole').length,
      auto: ws.reduce((n, w) => n + doneForYou(w).auto, 0),
      steps: ws.reduce((n, w) => n + w.steps.length, 0),
    };
  });
}
 ...
 *
 * If Growth ever came back empty it would mean SPEC had quietly become a job-management system
 * with a safety module bolted on — which is most of this market, and the thing it is meant not to
 * be. Cheap to check, and it checks an assumption nobody would think to question.
 */
export const emptyStreams = (list: readonly Workflow[] = WORKFLOWS): StreamKey[] =>
  STREAM_KEYS.filter(k => ownedBy(k, list).length === 0);
