...
export const CAPABILITIES: Capability[] = [
  ...
  C('jobs', 'mobile', 'Field app', 'Job card, SWMS, photos, materials used and client sign-off on the phone.', 'yes', '/tech-day — the SWMS, photos, materials used and the client’s sign-off, one button at a time in a fixed order. Materials land on the job cost. A photo goes into private storage and shows on the job under From site, readable only by the business that took it — and where no file store is connected the phone says the picture stays on it rather than pretending otherwise.', 'job_management'),
