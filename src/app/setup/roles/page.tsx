    ...
    <Shell title="Org chart — roles first" subtitle="Define the roles the business needs. Nobody is named yet.">
      <div className="grid gap-8 md:grid-cols-2">
          ...
          <ul className="mt-2 divide-y rounded-lg border bg-white">
            {roles.map(r => (
              <li key={r.id} className="flex items-center justify-between p-3 text-sm">
                <div><span className="font-medium">{r.title}</span> <span className="ml-2 rounded bg-cream px-1.5 text-xs uppercase">{r.stream} · {r.level}</span>
                  <div className="text-xs text-ink-light">reports to {roles.find(x => x.id === r.reportsToRoleId)?.title ?? '—'} · {r.holder?.name ?? 'vacant'}</div></div>
                {r.level !== 'gm' && <form action={removeRole}><input type="hidden" name="roleId" value={r.id} /><button className="text-xs text-ink-light/60 hover:text-red-700">remove</button></form>}
              </li>
            ))}
          </ul>
        </section>
        <section>
          ...
          <p className="mt-1 text-sm text-ink-light">Every business needs the numbers owned (Commercial), the work owned (Operations) and the clients owned (Growth). Add each with one click; the role comes with two KPIs per pillar you'll tune next.</p>
          <div className="mt-3 space-y-2">
            {proposals.map(t => (
              <form key={t.template_id} action={addRole} className="flex items-center justify-between rounded-lg border bg-white p-3 text-sm">
                ...
                <span><b>{t.title}</b> <span className="text-ink-light">· reports to {gm?.title}</span></span>
                <button className="rounded bg-rust px-3 py-1 text-white hover:bg-rust-dark">Add</button>
              </form>
            ))}
            ...
            {ops && (
              <form action={addRole} className="flex items-center justify-between rounded-lg border bg-white p-3 text-sm">
                ...
                <span><b>Supervisor</b> <span className="text-ink-light">· reports to {ops.title}</span></span>
                <button className="rounded bg-rust px-3 py-1 text-white hover:bg-rust-dark">Add</button>
              </form>
            )}
          </div>
          ...
          <form action={addRole} className="mt-2 grid gap-2 rounded-lg border bg-white p-3 text-sm">
            ...
            <button className="rounded bg-rust px-3 py-1 text-white hover:bg-rust-dark">Add custom role (no KPIs yet)</button>
          </form>
        </section>
      </div>
