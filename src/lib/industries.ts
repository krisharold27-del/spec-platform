 ...
 *
 * ── Renamed from `sectors`, 25 September ─────────────────────────────────────────────────────────
 *
 * Kris settled the vocabulary: *"streams commercial, operations and growth - sectors industrial,
 * commercial, mining and renewables but this is jbi"*. A SECTOR is a market one business works in,
 * and it is that business's own — JBI has four. This file was never about those; it is about what
 * KIND of business somebody is, which its own first line has said all along.
 *
 * So the name moved to where Kris's word belongs (`lib/sectors`) and this took the one that
 * describes it. The public `/sectors` page keeps its address: it speaks to people who have not
 * signed up yet, for whom "sectors" means industries in the ordinary way, and breaking an inbound
 * link to rename something nobody outside the code can see would be a poor trade.
 *
...
export const industryByKey = (key: string | null | undefined): Industry =>
  INDUSTRIES.find(s => s.key === key) ?? INDUSTRIES[0];
