...
/*
  Which table points at which — the delete order's source of truth.

  The implementation is in `schema-refs.mts` rather than here, because the journeys' clean-up is a
  plain .mjs run by node, and node cannot follow this file's extensionless `from '../db/schema'`.
  Re-exported so the application has one place to look.

  Read the note in that file for why the order is NOT asked of the database: this build deliberately
  emits no foreign keys, so CI's database and production's have none to find.
*/
export { referenceEdges, type Edge } from './schema-refs.mts';
