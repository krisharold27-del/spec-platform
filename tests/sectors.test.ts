...
import {
  WORK_KINDS, WORK_KEYS, isWorkKind, isProjectLike, EXAMPLE_SECTORS,
...
const job = (sectorId: string | null, kind: 'maintenance' | 'project' | 'shutdown' | null, valueCents: number, won = true) =>
  ({ sectorId, kind, valueCents, won, at: '2026-06-01T00:00:00.000Z' });

describe('the kinds of work are fixed, and the sectors are the business’s own', () => {
  it('is maintenance, project and shutdown — not a business’s to change', () => {
    /*
      Unlike the sectors, this is not a description of a market. It is a description of how work
  ...
  it('JBI’s four are an example, never a default', () => {
    /*
      A business shown four sectors it did not choose will keep them, and then SPEC has quietly
      decided what markets somebody works in.
    */
    expect([...EXAMPLE_SECTORS]).toEqual(['Industrial', 'Commercial', 'Renewables', 'Mining']);
    expect(mixOf([], [job('mining', 'project', 100)])).toEqual([]);
  });
});

describe('where the work actually is', () => {
  it('divides won work across the sectors, biggest first', () => {
    const mix = mixOf(JBI, [
  ...
  it('shares are of won work, never of the pipeline', () => {
    /* A sector with an enormous quote out has not won anything, and must not read as if it has. */
    const mix = mixOf(JBI, [
...
describe('four names on the letterhead, one paying for everything', () => {
  ...
  it('is happy with a real spread', () => {
    ...
    expect(c.says).toMatch(/spread across 4 sectors/);
  });

  it('says a one-sector business is exposed without calling it a mistake', () => {
...
describe('is each sector still being fed?', () => {
  it('names the sector that is starving while the total looks fine', () => {
    /*
  ...
  it('a sector with nothing at all is quiet, not starving', () => {
    /* Winding a sector down deliberately is a decision, not a fault, and must not read as a fault. */
...
describe('how much of the year is there before anybody sells anything', () => {
  ...
  it('sets no target, because the right share depends on the market', () => {
    ...
    const src = require('node:fs').readFileSync('src/lib/sectors.ts', 'utf8');
...
describe('a shutdown window does not move', () => {
  const sd = (over = {}) => ({
    id: 's1', title: 'Mill shutdown', sectorId: 'mining',
